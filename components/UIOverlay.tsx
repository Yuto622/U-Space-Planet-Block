...
import { Box, Globe, Sun, Circle, Disc, Code2, Wand2, Hammer, FolderOpen, ChevronUp, FileJson, History, Play, Pause, Info, Wrench, Loader2 } from 'lucide-react';
...
interface UIOverlayProps {
  ...
  onDismantle: () => void;
  onRebuild: (type: 'Earth' | 'Sun' | 'Mars' | 'Saturn') => void;
  onNewScene: (type: 'Earth' | 'Sun' | 'Mars' | 'Saturn') => void;
...
export const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  // Show default presets if current model is one of the base planets
  const isBasePlanet = ['Earth', 'Sun', 'Mars', 'Saturn'].includes(currentBaseModel);

  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none select-none text-slate-800">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex flex-col gap-2">
            ...
            >
                ...
                <DropdownItem onClick={() => onNewScene('Earth')} icon={<Globe size={16}/>} label="Earth" />
                <DropdownItem onClick={() => onNewScene('Mars')} icon={<Circle size={16}/>} label="Mars" />
                <DropdownItem onClick={() => onNewScene('Saturn')} icon={<Disc size={16}/>} label="Saturn" />
                <DropdownItem onClick={() => onNewScene('Sun')} icon={<Sun size={16}/>} label="Sun" />
            ...
            {isDismantling && !isGenerating && (
                <div className="flex items-end gap-4 animate-in slide-in-from-bottom-10 fade-in duration-300">
                     ...
                     >
                        ...
                        {/* Standard Presets */}
                        <DropdownItem onClick={() => onRebuild('Earth')} icon={<Globe size={18}/>} label="Earth" />
                        <DropdownItem onClick={() => onRebuild('Sun')} icon={<Sun size={18}/>} label="Sun" />
                        <DropdownItem onClick={() => onRebuild('Mars')} icon={<Circle size={18}/>} label="Mars" />
                        <DropdownItem onClick={() => onRebuild('Saturn')} icon={<Disc size={18}/>} label="Saturn" />
                        <div className="h-px bg-slate-100 my-1" />
