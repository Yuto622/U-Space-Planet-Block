...
const App: React.FC = () => {
  ...
  const [currentBaseModel, setCurrentBaseModel] = useState<string>('Earth');
  ...
  useEffect(() => {
    ...
    // Initial Model Load
    engine.loadInitialModel(Generators.Earth());
  ...
  const handleNewScene = (type: 'Earth' | 'Sun' | 'Mars' | 'Saturn') => {
  ...
  const handleRebuild = (type: 'Earth' | 'Sun' | 'Mars' | 'Saturn') => {
    const generator = Generators[type];
    if (generator && engineRef.current) {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: `
                    ...
                    Strict Rules:
                    1. Use approximately 150 to 800 voxels.
                    2. The model must be centered at x=0, y=0, z=0.
                    3. Ensure the structure is physically plausible (connected).
                    4. Coordinates should be integers.
